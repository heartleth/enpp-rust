...
pub fn first_phrase(s :&Vec<String>, is_first :bool, allow_multi :bool)->usize {
    ...
    if len == 1 {
        ...
        if is_bracket(&s.join(" "), ('(', ')')) || is_bracket(&s.join(" "), ('{', '}')) {
            ...
            for elem in 1 .. s.len() - 1 {
                if s[elem] == "|" {
                    ret = elem + 1;
                    return ret + first_phrase(&s[ret..].to_vec(), true, allow_multi);
                }
            }
        }
            ...
            ret = 2;
            ret += first_phrase(&s[2..].to_vec(), false, allow_multi);
        }
        else if regi(&first_low, r"^[a-zA-Z_]\w*:$") {
            ret = 1;
            ret += first_phrase(&s[1..].to_vec(), false, allow_multi);
        }
